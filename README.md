...
Framex does not introduce a new semantic theory. FrameNet is the
computational-lexicographic application of **Frame Semantics**, the linguistic
theory developed by **Charles J. Fillmore** in the mid-1970s. Fillmore’s 1975
work introduced the approach and his 1976 article, [*Frame Semantics and the
Nature of Language*](https://doi.org/10.1111/j.1749-6632.1976.tb25467.x), gave
its classic formulation. The word *frame* also appeared in related work by
other scholars at the time; Fillmore’s distinctive contribution was relating
word meanings to the structured situations that they evoke.
...
| --- | --- |
| `Framex.Corpus` | The opened artifact and its lazily read annotation shards. |
| `Framex.Frame` | Frames, their definitions, and their frame elements. |
| `Framex.Lu` | Lexical units, frame membership, search, and valence patterns. |
| `Framex.Fe` | A named frame element within a frame. |
| `Framex.Annotation` | Exemplars, sentences, annotated arrangements, and omitted roles. |
| `Framex.Relation` | Typed frame relations and inheritance paths. |
| `Framex.SemType` | Semantic types attached to frames, frame elements, and lexical units. |
